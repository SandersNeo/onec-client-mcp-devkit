...
Если путь в `runMcp` заполнен, он читается как JSON-конфигурация. В JSON используется ключ `port`, а в строке `/C` - `mcpPort`; значение из `/C` имеет приоритет. Если порт не указан, используется `8080`.
