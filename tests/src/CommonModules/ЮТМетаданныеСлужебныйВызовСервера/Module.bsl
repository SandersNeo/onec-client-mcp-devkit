...
Функция ОписаниеОбъектаМетаданных(Знач Значение, ЗаполнятьРеквизиты = Истина) Экспорт
	...
	Если (ОписаниеТипа.ИмяКоллекции = "РегистрыБухгалтерии" Или ОписаниеТипа.ИмяКоллекции = "AccountingRegisters")
		И МетаданныеОбъекта.Корреспонденция Тогда
		ТипПланСчета = ЮТТипыДанныхСлужебный.НовыйТипСсылки(МетаданныеОбъекта.ПланСчетов);
		ОписаниеТипаПланСчета = Новый ОписаниеТипов(ЮТКоллекции.ЗначениеВМассиве(ТипПланСчета));
		ДобавитьСтандартныеРеквизитыДтКтПриКорреспонденцииСчетов(ОписаниеТипаПланСчета, ОписаниеОбъект.Реквизиты);
	КонецЕсли;
...
Процедура ДобавитьСтандартныеРеквизитыДтКтПриКорреспонденцииСчетов(ОписаниеТипаПланСчета, ОписаниеОбъектРеквизиты)
	
	Если ЮТОкружение.ИспользуетсяАнглийскийВстроенныйЯзык() Тогда
		ИмяРеквизитаСчет = "Account";
		ИмяРеквизитаВидСубконто = "ExtDimensionType";
		ИмяРеквизитаСубконто = "ExtDimension";
		СуффиксДт = "Dr";
		СуффиксКт = "Cr";
	Иначе
		ИмяРеквизитаСчет = "Счет";
		ИмяРеквизитаВидСубконто = "ВидСубконто";
		ИмяРеквизитаСубконто = "Субконто";
		СуффиксДт = "Дт";
		СуффиксКт = "Кт";
	КонецЕсли;
	СуффиксыДтКт = Новый Массив();
	СуффиксыДтКт.Добавить(СуффиксДт);
	СуффиксыДтКт.Добавить(СуффиксКт);
	
	// добавление СчетДтКт
	Для Каждого Вид Из СуффиксыДтКт Цикл
		Реквизит = ЮТМетаданные.ОписаниеРеквизита();
		Реквизит.Имя = ИмяРеквизитаСчет + Вид;
		Реквизит.Тип = ОписаниеТипаПланСчета;
		Реквизит.Обязательный = Ложь;
		Реквизит.ЭтоКлюч = Ложь;
		Реквизит.Стандартный = Истина;
		ОписаниеОбъектРеквизиты.Вставить(Реквизит.Имя, Реквизит);
	КонецЦикла;
		
	// добавление ВидСубконтоДтКт и удаление ВидСубконто
	// добавление СубконтоДтКт и удаление Субконто
	ДобавляемыеОписания = Новый Структура();
	УдаляемыеОписания = Новый Массив();
	МассивМаркеров = Новый Массив();
	МассивМаркеров.Добавить(ИмяРеквизитаВидСубконто);
	МассивМаркеров.Добавить(ИмяРеквизитаСубконто);
	Для Каждого КлючЗначение Из ОписаниеОбъектРеквизиты Цикл
		ОписаниеРеквизита = КлючЗначение.Значение;
		Для Каждого Элемент Из МассивМаркеров Цикл
			Если Не СтрНачинаетсяС(ОписаниеРеквизита.Имя, Элемент) Тогда
				Продолжить;
			КонецЕсли;
			
			Для Каждого Вид Из СуффиксыДтКт Цикл
				Реквизит = ЮТМетаданные.ОписаниеРеквизита();
				Реквизит.Имя = СтрЗаменить(ОписаниеРеквизита.Имя, Элемент, Элемент + Вид);
				Реквизит.Тип = ОписаниеРеквизита.Тип;
				Реквизит.Обязательный = ОписаниеРеквизита.Обязательный;
				Реквизит.ЭтоКлюч = ОписаниеРеквизита.ЭтоКлюч;
				Реквизит.Стандартный = ОписаниеРеквизита.Стандартный;
				ДобавляемыеОписания.Вставить(Реквизит.Имя, Реквизит);
			КонецЦикла;
			
			УдаляемыеОписания.Добавить(КлючЗначение.Ключ);
		КонецЦикла;
	КонецЦикла;
	
	Для Каждого Элемент Из ДобавляемыеОписания Цикл
		ОписаниеОбъектРеквизиты.Вставить(Элемент.Ключ, Элемент.Значение);
	КонецЦикла;
	Для Каждого Элемент Из УдаляемыеОписания Цикл
		ОписаниеОбъектРеквизиты.Удалить(Элемент);
	КонецЦикла;
