...
Процедура ЗаписатьДвиженияРегистратора(Регистратор, Данные, МетаданныеРегистра, ПараметрыЗаписи, ОбщиеРеквизиты = Неопределено) Экспорт
	...
	// делаем через загрузить, т.к. субконто у регистра бухгалтерии нельзя заполнить через ЗаполнитьЗначенияСвойств
	ЗаполнитьДвиженияЧерезЗагрузить = ЮТМетаданные.ЭтоРегистрБухгалтерии(МетаданныеРегистра);
	
	ДатаРегистратора = ЮТЗапросы.ЗначениеРеквизита(Регистратор, "Дата");
	НаборЗаписей.Отбор.Регистратор.Установить(Регистратор);
	
	Контейнер = НаборЗаписей;
	Если ЗаполнитьДвиженияЧерезЗагрузить Тогда
		Контейнер = Новый ТаблицаЗначений();
		Для Каждого Элемент Из МетаданныеРегистра.Реквизиты Цикл
			Контейнер.Колонки.Добавить(Элемент.Ключ, Элемент.Значение.Тип);
		КонецЦикла;
	КонецЕсли;

	ИмяРеквизитаАктивность = ?(АнглийскийЯзык, "Active", "Активность");
	Для Каждого Строка Из Данные Цикл
		Запись = Контейнер.Добавить();
		Запись[ИмяРеквизитаАктивность] = Истина; // установка значения по умолчанию для режима ЗаполнитьДвиженияЧерезЗагрузить
	...
	Если ЗаполнитьДвиженияЧерезЗагрузить Тогда
		НаборЗаписей.Загрузить(Контейнер);
	КонецЕсли;
